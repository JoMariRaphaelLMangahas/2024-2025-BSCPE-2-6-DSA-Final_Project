...
class TowerOfHanoi:
    ...
    def draw_congratulations(self):
        ...
        # Draw buttons
        restart_rect = pygame.Rect(WIDTH // 2 - 140, HEIGHT // 2 - 60, 280, 50)
        new_game_rect = pygame.Rect(WIDTH // 2 - 140, HEIGHT // 2, 280, 50)
        quit_rect = pygame.Rect(WIDTH // 2 - 140, HEIGHT // 2 + 60, 280, 50)
        ...
        screen.blit(restart_text, restart_rect.move(90, 10))
        screen.blit(new_game_text, new_game_rect.move(75, 10))
        screen.blit(quit_text, quit_rect.move(110, 10))

        # Save the buttons in self.last_popup
        self.last_popup = ('congratulations', [restart_rect, new_game_rect, quit_rect])
    ...
    def draw_popup(self, question_text, yes_callback=None, no_callback=None):
        pygame.draw.rect(screen, GRAY, (WIDTH // 2 - 150, HEIGHT // 2 - 100, 300, 200))
        question_rendered = font.render(question_text, True, BLACK)
        screen.blit(question_rendered, (WIDTH // 2 - question_rendered.get_width() // 2, HEIGHT // 2 - 70))
    ...
    def draw_restart_popup(self):
        yes_rect, no_rect = self.draw_popup("You will lose progress. Restart?")
        self.last_popup = ('restart', yes_rect, no_rect)

    def draw_new_game_popup(self):
        yes_rect, no_rect = self.draw_popup("Start a new game?")
        self.last_popup = ('new_game', yes_rect, no_rect)

    def draw_quit_popup(self):
        yes_rect, no_rect = self.draw_popup("Want to quit the game?")
        self.last_popup = ('quit', yes_rect, no_rect)
    ...
    def handle_click(self, x, y):        
        ...
        if self.last_popup:
            popup_type, buttons = self.last_popup
            
            if popup_type == 'congratulations':
                restart_rect, new_game_rect, quit_rect = buttons 
                if restart_rect.collidepoint(x, y):
                    self.reset_game(self.saved_shuffled_disks)  # Restart with the current shuffled state
                elif new_game_rect.collidepoint(x, y):
                    self.reset_game()  # Start a new game with a new random shuffle
    ...
    def handle_popup_click(self, x, y, yes_rect, no_rect, yes_action, no_action):
